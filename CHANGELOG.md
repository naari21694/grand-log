...
- OpenSSF Scorecard runs only when the repository is public (its checkout fails on a private repo), matching the CodeQL gate, so CI stays clean while private.
- A pre-commit hook blocks committing personal-data or secret files (`work/`, `.env`, cookies, the local items index), on top of the existing private-key check.
...
- Backfill rate-limit guidance added consistently to `.env.example`, the README, INSTALL, and CONFIGURATION: the Gemini free tier is too small for a full backlog, so use Groq or a local Ollama for the backfill and keep Gemini for live shares.
- IDEAS.md reconciled with shipped work and given a public-launch and community section.
...
[Unreleased]: https://github.com/naari21694/grand-log/compare/v0.4.1...HEAD
[0.4.1]: https://github.com/naari21694/grand-log/releases/tag/v0.4.1
